...
# Round and store the final GDP raster and polygon outputs in results_final/,
# ready for Zenodo archiving and public release.
...
v_gdp_pc_adm0 <- v_adm0 %>%
  mutate(
    across(where(is.numeric) & !matches("slope"), ~round(., 0)),
    slope = round(slope, 5)
  ) %>%
  select(-cntry_id)
...
cntry_names <- v_gdp_pc_adm0 %>% as_tibble() %>% select(iso3, Country)
