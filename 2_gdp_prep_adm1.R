### 2_gdp_prep_adm1.R
# Process subnational (admin-1) GDP per capita data from OECD, DOSE and other sources,
# match regions to GADM boundaries, interpolate ratios to national GDP, and save results.
...
myFunDose <- function(iso3_name = 'KAZ'){
  ...
  iso3_gdp_pc <- read_csv('data_in/data_update_nov2025/DOSE_V2.11.csv') %>%
...
IND_gdp_pc <-  myFunDose('IND')

IRN_gdp_pc <-  myFunDose('IRN')

MYS_gdp_pc <-  myFunDose('MYS')

MNG_gdp_pc <-  myFunDose('MNG')

NGA_gdp_pc <-  myFunDose('NGA')

PAK_gdp_pc <-  myFunDose('PAK')

PRY_gdp_pc <-  myFunDose('PRY') # income, not GDP per capita

ZAF_gdp_pc <-  myFunDose('ZAF')
...
subnat_data <- read.xlsx('data_in/gadm_level1_gdp_pc.xlsx', sheet='gadm_level1_meta', startRow = 1) %>%
  ...
  select('iso3','GID_1','Subnat',as.character(1988:2024)) %>%
  arrange(iso3)
...
cols.num <- c(as.character(1988:2024))
...
subnat_data <- read.xlsx('data_in/nuts2_gdp_pps.xlsx', sheet='Sheet 1', startRow = 8) %>%
  ...
  select(iso3, 'GEO.(Codes)',as.character(2010:2022)) %>%
  mutate(across(as.character(2010:2022), as.numeric ))

subnat_gis_data <-  read_sf("data_gis/NUTS_RG_01M_2021_4326.gpkg") %>%
  filter(LEVL_CODE == 2) %>%
...
subnat_data <- read_csv('data_in/OECD.CFE.EDS,DSD_REG_ECO@DF_GDP,2.0,filtered,2025-09-23 09-28-21.csv') %>%
  as_tibble() %>%
  rename(Subnat = 'Reference area') %>%
  rename(REG_ID = REF_AREA) %>%
  select(REG_ID, Subnat, TERRITORIAL_LEVEL, TIME_PERIOD, OBS_VALUE) %>%
  pivot_wider(names_from = TIME_PERIOD, values_from = OBS_VALUE) %>%
  select(REG_ID, Subnat, TERRITORIAL_LEVEL, as.character(1995:2023)) %>%
  mutate(reg2 = substr(REG_ID,1,2) ) %>%
...
subnat_gis_data <- gis_data %>%
  ...
  mutate(GID_nmbr = (1000+cntry_id)*1000+rowNmbr) %>%
  select(-rowNmbr)%>%
  select(Country,iso3,cntry_id,Subnat,GID_1,GID_nmbr,as.character(c(1995:2023)))
...
# oecd_gis_data <- read_sf('/Users/mkummu/R/migration_data_bee/data_in/OECD_TL3_2020_fixed_valid.gpkg')%>%
#   #st_drop_geometry() %>%
...
n_sf_oecd_tl3_noGeom <- sf_oecd_tl3_noGeom %>%
  group_by(iso3) %>%
  summarise(unique_iso3 = n())
...
adm2_oecd <- read_csv('data_in/OECD.CFE.EDS,DSD_REG_ECO@DF_GDP,2.0,filtered,2025-09-23 09-28-21.csv') %>%
  as_tibble() %>%
  rename(Subnat = 'Reference area') %>%
  rename(REG_ID = REF_AREA) %>%
  rename(year = TIME_PERIOD) %>%
  select(REG_ID, TERRITORIAL_LEVEL, year, OBS_VALUE) %>%
  filter(TERRITORIAL_LEVEL == 'TL3') %>%
  left_join(sf_oecd_tl3_noGeom %>% rename(REG_ID = tl3_id)) %>%
  arrange(REG_ID, year )



adm2_oecd_wider <- adm2_oecd %>%
  pivot_wider(names_from = year, values_from = OBS_VALUE) %>%
  select(iso3, REG_ID, name_en, as.character(1995:2023)) %>%
  mutate(reg2 = substr(REG_ID,1,2) )

n_adm2_oecd_wider <- adm2_oecd_wider %>%
  filter(!is.na(iso3)) %>%
  group_by(iso3) %>%
  summarise(unique_iso3_data = n()) %>%
  left_join(n_sf_oecd_tl3_noGeom) %>%
  mutate(check = ifelse(unique_iso3_data == unique_iso3, 'Same', 'NA'))

iso3_oecd_2025 <- n_adm2_oecd_wider %>%
  filter(check == "Same") %>%
  # omit also Finland and Korea; let's use old data for them due to some discontinuity of some admin units
  filter(!iso3 %in% c('FIN', 'KOR', 'NLD'))

### proceed to data

subnat_data_oecd_2025 <- adm2_oecd_wider %>%
  as_tibble() %>%
  rename(Subnat = name_en) %>%
  select(c(REG_ID, Subnat, as.character(1995:2023))) %>%
  #mutate(reg2 = substr(REG_ID,1,2) ) %>%
  left_join(meta_OECD3,by='REG_ID') %>%
  left_join(cntryID,by='iso3') %>%
  # only those that mach with the gis data, for other we use old OECD data
  dplyr::filter(iso3 %in% iso3_oecd_2025$iso3) %>%
  dplyr::filter(iso3 %in% cntryList_OECD3$iso3) %>%
  # germany, portugal omit few 2023 entries
  rename(yr2023 = '2023') %>%
  mutate(yr2023 = ifelse(iso3 %in% c('DEU','PRT'), NA, yr2023)) %>%
  rename('2023' = yr2023) %>%
  # latvia, portugal omit few 2023 entries
  rename(yr2022 = '2022') %>%
  mutate(yr2022 = ifelse(iso3 %in% c('LVA'), NA, yr2022)) %>%
  rename('2022' = yr2022) %>%
  # in germany few year 2022 missing; let's assume stays same as previous year
  # this is trend in most of the admin units
  rename(yr2022 = '2022') %>%
  rename(yr2021 = '2021') %>%
  mutate(yr2022 = ifelse(iso3 %in% c('DEU') & is.na(yr2022), yr2021, yr2022)) %>%
  rename('2022' = yr2022) %>%
  rename('2021' = yr2021) %>%
  #select(-c(iso2, reg2)) %>%
...
gis_data <- read_sf('/Users/mkummu/R/migration_data_bee/data_in/OECD_TL3_2020_fixed_valid.gpkg')%>%
  ...
  left_join(cntryID,by='iso3') %>%
  select(iso3,tl3_id) %>%
  dplyr::filter(iso3 %in% unique(subnat_data_oecd_2025$iso3)) %>%
  arrange(iso3, tl3_id)

id_gis <- unique(gis_data$iso3) %>% sort
id_subnat <- unique(subnat_data_oecd_2025$iso3) %>%
  sort()

compare::compare(id_subnat,id_gis)

subnat_gis_data_oecd2025 <- gis_data %>%
  left_join(subnat_data_oecd_2025,by=c('tl3_id','iso3')) %>%
  #st_drop_geometry() %>%
  arrange(iso3,Subnat) %>%
  group_by(iso3) %>%
  mutate(rowNmbr = row_number()) %>%  # for each subnat in a country a running number from 1...
...
# subnat_gis_data_OECD3_2025 <- subnat_gis_data
temp_oecd3_2025 <- subnat_gis_data_oecd2025 %>%
  st_drop_geometry()

subnat_data_OECD3_2025 <- subnat_data_oecd_2025




### 3.5 OECD level 3 - OLD DATA ----
...
subnat_data <- read.xlsx('data_in/adm2_oecd_level3_wide.xlsx', sheet='adm2_oecd_wide', startRow = 1) %>%
  ...
  left_join(cntryID,by='iso3') %>%
  dplyr::filter(iso3 %in% cntryList_OECD3$iso3) %>%
  # remove those that are in 2025 data

  dplyr::filter(!iso3 %in% unique(subnat_data_OECD3_2025$iso3)) %>%
  #select(-c(iso2, reg2)) %>%
  select(Country, iso3, cntry_id,REG_ID, everything()) %>%
  arrange(REG_ID) %>%
  rename(tl3_id = REG_ID) %>%
...
### Let's join the new oecd with this old one for Korea

subnat_data_KOR <- subnat_data %>%
  filter(iso3 == "KOR") %>%
  left_join(adm2_oecd_wider
            %>% filter(iso3 == 'KOR')
            %>% select(REG_ID,paste0(2021:2023))
            %>% rename(tl3_id = REG_ID))


subnat_data_comb <- subnat_data %>%
  filter(!iso3 == "KOR") %>%
  bind_rows(subnat_data_KOR)


id_subnat <- unique(subnat_data_comb$iso3) %>%
  sort()
...
gis_data <- read_sf('/Users/mkummu/R/migration_data_bee/data_in/OECD_TL3_2020_fixed_valid.gpkg')%>%
  ...
  left_join(cntryID,by='iso3') %>%
  select(iso3,tl3_id) %>%
  dplyr::filter(iso3 %in% subnat_data_comb$iso3) %>%
  arrange(iso3, tl3_id)
...
subnat_gis_data <- gis_data %>%
  left_join(subnat_data_comb,by=c('tl3_id','iso3')) %>%
  #st_drop_geometry() %>%
  arrange(iso3,Subnat) %>%
  group_by(iso3) %>%
  # remove those that are in 2025 data
  ...
  mutate(GID_nmbr = (1000+cntry_id)*1000+rowNmbr) %>%
  select(-rowNmbr)%>%
  select(Country,iso3,cntry_id,Subnat,GID_1,GID_nmbr,as.character(c(1990:2023)))

subnat_gis_data_OECD3 <- subnat_gis_data
temp_oecd3 <- subnat_gis_data_OECD3 %>%
  st_drop_geometry()

subnat_data_OECD3 <- subnat_data_comb
...
subnat_gis_combined <- bind_rows(subnat_gis_data_hist,
                                 ...
                                 subnat_gis_data_oecd2025,
                                 subnat_gis_data_OECD3,
                                 subnat_gis_data_GADM,
                                 subnat_gis_data_BHS) %>%
  select(Country,iso3,cntry_id,Subnat,GID_1,GID_nmbr,as.character(1989:2023)) %>%
...
v_subnatPop_comb <- sf_subnat_gis_combined %>%
  ...
  #select(-ID) %>%
  set_names(c('GID_nmbr', 'cntry_id', paste0('pop',1989:2024))) %>%
...
sf_gdp <- sf_subnat_gis_combined %>%
  select(GID_nmbr, as.character(1989:2023)) %>%
  #mutate('2022' = NA) %>%
  as_tibble()

# weight with population

sf_gdp_pop_weighted <- (sf_gdp %>% select(`1989`:`2023`)) *
  (v_subnatPop_comb %>% select(pop1989:pop2023))


sf_gdp_pop_weighted <-  sf_gdp_pop_weighted %>%
  as_tibble() %>%
  #select(-GID_nmbr) %>%
  bind_cols(sf_subnat_gis_combined%>% select(cntry_id)) %>%
  select(cntry_id, as.character(1989:2023))
...
sf_adm0_pop <- v_subnatPop_comb %>%
  select(-GID_nmbr) %>%
  select(cntry_id, pop1989:pop2023) %>%
  group_by(cntry_id) %>%
  summarise(across(everything(), list(sum)))
...
sf_adm0_gdp  <- sf_adm0_gdp %>%
  ...
  set_names('cntry_id', 'iso3', as.character(1989:2023))
...
# adm1_gdp_ratio_interp_ext <- adm1_gdp_ratio_interp %>%
#   pivot_wider(names_from = year, values_from = gdpRatio) %>%
#   rename(yr2023 = '2023') %>%
#   mutate('2024' = yr2023) %>%
#   rename('2023' = yr2023) %>%
#   pivot_longer(-c(cntry_id, iso3), names_to = 'year', values_to = 'gdp')
