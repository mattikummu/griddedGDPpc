...
# Combine admin-0 and admin-1 GDP data with spatial polygons, rasterise to 5 arc-min,
# and write GeoPackage and raster outputs used by the downscaling steps.
...
if (file.exists('data_gis/gdp_Adm0Adm1_raster_5arcmin_feb2024.tif')){
  ...
  # simplify first polygon


  source('functions/f_simplify_polyg.R')


  f_simplify_polyg(input_gpkg_file = 'data_gis/gdp_adm0adm1_polyg_feb2024.gpkg',
                   output_gpkg_file = 'data_gis/gdp_adm0adm1_polyg_feb2024_simpl.gpkg',
                   simplify_percentage = 2.5)

  v_gdp_adm0adm1_polyg_simpl <- vect('data_gis/gdp_adm0adm1_polyg_feb2024_simpl.gpkg')

  r_gdp_adm0adm1_polyg_1arcmin_terra <- terra::rasterize(x = v_gdp_adm0adm1_polyg_simpl,
                                                         y = rast(ref_raster_1arcmin),
                                                         field = "GID_nmbr")
...
DataAdm0 = adm0_comb_interpExtrap
DataAdm1 = adm1_ratioAdm1Adm0_interp
# pop
r_popCount_mod_ext <- rast('data_gis/r_pop_GHS_1989_2024_5arcmin.tif')
ext_pop <- exactextractr::exact_extract(x= r_popCount_mod_ext,y=gdp_adm0adm1_polyg, fun='sum')
dim(ext_pop)
ext_pop_long <- gdp_adm0adm1_polyg %>%
  st_drop_geometry() %>%
  bind_cols(ext_pop) %>%
  select(GID_nmbr, sum.pop1990:sum.pop2024) %>%
  set_names(c('GID_nmbr', paste0(1990:2024))) %>%
  pivot_longer(names_to = 'year', values_to = 'popCount', -c(GID_nmbr))

DataAdm1_pop <- DataAdm1 %>%
  left_join(DataAdm0) %>%
  filter(year > 1989) %>%
  mutate(gdpAdm1 = gdp*gdpRatio) %>%
  mutate(year = as.character(year)) %>%
  left_join(ext_pop_long)

adm0gdp_dataAdm1 <- DataAdm1_pop %>%
  mutate(gdpTot = gdpAdm1* popCount) %>%
  group_by(year, iso3) %>%
  summarise(gdpAdm0_fromAdm1 = sum(gdpTot, na.rm = TRUE), popAdm0_fromAdm1 = sum(popCount, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(gdp0_fromAdm1Gdp = gdpAdm0_fromAdm1 / popAdm0_fromAdm1) %>%
  left_join(DataAdm0 %>% mutate(year = as.character(year))) %>%
  mutate(harmon_ratio = gdp0_fromAdm1Gdp / gdp)

adm1_ratioAdm1Adm0_interp_harm <- adm1_ratioAdm1Adm0_interp %>%
  left_join(adm0gdp_dataAdm1 %>% select(year, iso3, harmon_ratio) %>% mutate(year = as.numeric(year))) %>%
  mutate(gdpRatio_harm = gdpRatio / harmon_ratio) %>%
  select(-gdpRatio, -harmon_ratio) %>%
  rename(gdpRatio = gdpRatio_harm)


## test that done correctly

t_DataAdm1_pop <- adm1_ratioAdm1Adm0_interp_harm %>%
  left_join(DataAdm0) %>%
  filter(year > 1989) %>%
  mutate(gdpAdm1 = gdp*gdpRatio) %>%
  mutate(year = as.character(year)) %>%
...
t_adm0gdp_dataAdm1 <- t_DataAdm1_pop %>%
  mutate(gdpTot = gdpAdm1* popCount) %>%
  group_by(year, iso3) %>%
  summarise(gdpAdm0_fromAdm1 = sum(gdpTot, na.rm = TRUE), popAdm0_fromAdm1 = sum(popCount, na.rm = TRUE)) %>%
  ungroup() %>%
...
rast_gdp <- f_gdp_data2raster_adm1(inYears = yearsIn,
                                  ...
                                  inDataAdm1 = adm1_ratioAdm1Adm0_interp_harm)

terra::writeRaster(rast_gdp,paste0('results/rast_adm1_','gdp','_perCapita','_',yearsIn[1],'_',yearsIn[length(yearsIn)],
...
### 6. load simplified  polygon layer ----

gdp_adm0adm1_polyg_simpl <- st_read('data_gis/gdp_adm0adm1_polyg_feb2024_simpl.gpkg') %>%
  select(GID_nmbr)
...
for (iVar in 1:length(varNames)) {
  ...
  vect_varName <- f_gdp_data2gpkg_adm1(inYears = 1990:2024,
