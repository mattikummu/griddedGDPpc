...
# Extract pixel-level predictors (population, urbanisation, Gini, travel time) per admin-1 unit
# and assemble the training dataset used by the MATLAB neural-network downscaling model.
...
if (file.exists('results/ext_data_adm1_mar2024.RData')){
  ...
  # r_popCount_mod has 36 layers (pop1989–pop2024); subset to 1990:2022 (33 layers) to match training targets
  r_popCount <- subset(r_popCount_mod, paste0('pop', 1990:2022))
  ...
  # r_gini has 32 layers (1990–2021); repeat last layer to cover 2022 → 33 layers for 1990:2022
...
adm0gini <- read.csv('data_in/gini_adm0_DispMkt_extrap.csv') %>%
  as_tibble() %>%
  select(-gini_mkt) %>%
  mutate(year = as.character(year))
...
### 5.4 downscaling data with shares for urb and travel time


adm1_polyg_comb_Matlab_reg_ratio <- adm1_polyg_comb_Matlab_reg %>%
  mutate(urbRatio = urb / adm0urb) %>%
  mutate(travelTimeRatio = travelTime/adm0travelTime) %>%
  select(-urb, -travelTime) %>%
  select(iso3, GID_nmbr, Subnat, year,  gdpRatio, urbRatio, travelTimeRatio , everything())

# 5.5 write file

write.csv(adm1_polyg_comb_Matlab_reg_ratio, 'downscalingMatlab/adm1DataForDownscaling_ratio.csv')
