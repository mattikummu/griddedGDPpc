### 9_adm2_validation.R
# Validate downscaled GDP per capita against independent OECD TL3 (admin-2) data
# and against reported national values; produce correlation plots and deviation statistics.
...
# r_popCount_mod has 36 layers (pop1989–pop2024); subset to 1990:2022 (33 layers) to match GDP rasters
r_popCount_mod <- subset(r_popCount_mod, paste0('pop', 1990:2022))
...
validation_gdp_adm0_adm2 <- ggpubr::ggscatter(sf_adm0_comb_adm2, x = "gdp_pc", y = "gdp_pc_downscaled",
                                      add = "reg.line", conf.int = TRUE,
                                      cor.coef = TRUE, cor.method =  "pearson",
                                      xlab = "Observed Value", ylab = "Downscaled Value") +
  geom_abline(intercept = 0, slope = 1)  +
...
### check trends in China -----

sf_data_adm2_chn <- read_csv('results/polyg_adm2_gdp_perCapita_1arcmin_1990_2022.csv') %>%
  filter(iso3 == "CHN") %>%
  select(iso3, GID_2,adm2ID, NAME_2, '2019':'2022') %>%
  mutate(change = (`2022` - `2019`) / `2019` * 100)

hist(sf_data_adm2_chn$change)


sf_data_adm2_aut <- read_csv('results/polyg_adm2_gdp_perCapita_1arcmin_1990_2022.csv') %>%
  filter(iso3 == "AUT") %>%
  select(iso3, adm2ID, NAME_2, '2019':'2022') %>%
  mutate(change = (`2022` - `2019`) / `2019` * 100)

sf_data_adm2_fin <- read_csv('results/polyg_adm2_gdp_perCapita_1arcmin_1990_2022.csv') %>%
  filter(iso3 == "FIN") %>%
  select(iso3, adm2ID, NAME_2, '2019':'2022') %>%
  mutate(change = (`2022` - `2019`) / `2019` * 100)


sf_data_adm1_aut <- read_csv('results/polyg_adm2_gdp_perCapita_1arcmin_1990_2022.csv') %>%
  filter(iso3 == "AUT") %>%
  select(iso3, GID_nmbr, Subnat, '2019':'2022') %>%
  mutate(change = (`2022` - `2019`) / `2019` * 100)
