...
f_gdp_data2gpkg_adm1 <- function(inYears = yearsIn, IndexName = 'gdp', 
  ...
  polyg_tempDataAdm0Adm1_wTrend <- gdp_adm0adm1_polyg_simpl %>% 
    left_join(tempDataAdm0Adm1_wTrend) %>% 
  ...
  st_write(polyg_tempDataAdm0Adm1_wTrend,paste0('results/polyg_adm1_',IndexName,'_perCapita_',
                                          inYears[1],'_',inYears[length(inYears)],'.gpkg'), delete_dsn=T)
  
  # only csv
  temp <- polyg_tempDataAdm0Adm1_wTrend %>% 
    st_drop_geometry() 
