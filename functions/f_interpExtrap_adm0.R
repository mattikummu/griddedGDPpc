...
f_interpExtrap_adm0 <- function(nameIndic, timestep) {
  ...
  # full timeseries
  dataFull <- indicWider %>%
    set_names(c('iso3', paste0('n',timestep))) %>%
    drop_na()
  ...
  thirdLast <- paste0('n',names(indicWider)[ncol(indicWider)-3])
  ...
  dataNearlyFull <- indicWider %>%
    ...
    mutate(nearFullTimeser = ifelse(!is.na(n1993)&!is.na(!!as.name(thirdLast)), 1, 0 )) %>%
  ...
  write_csv(dataFull_NearlyFull, 'results/dataFull_NearlyFull.csv')
  ...
  dataFull_NearlyFull_Rest_Less5 <- dataFull_NearlyFull %>%
    ...
    set_names('iso3', paste0(timestep)) %>%
