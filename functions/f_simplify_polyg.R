...
f_simplify_polyg <- function(input_gpkg_file, output_gpkg_file, simplify_percentage = 1) {
  ...
  tryCatch({
    ...
    # Write the simplified geometries to a temporary GeoJSON file
    
    simp_geojson_file <- file.path("data_gis/sf_adm_fixed_polyg_simp.geojson")
    simp_topoj_file <- file.path("data_gis/sf_adm_fixed_polyg_simp.topojson")
    
    sf::st_write(obj = sf_adm_fixed_polyg_topo, dsn = simp_geojson_file, driver = "GeoJSON", delete_dsn = TRUE)
    
    message("Step 7: Converting to TopoJSON using mapshaper...")
    # Construct and execute the mapshaper command to convert to TopoJSON
    mapshaper_path <- Sys.which("mapshaper")
    if (mapshaper_path == "") {
      stop("mapshaper command-line tool not found. Please ensure it is installed and in your PATH.")
